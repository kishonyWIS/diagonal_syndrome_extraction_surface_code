#!/usr/bin/env python3
"""Compare standard vs diagonal schedule memory circuits using tqec conventions."""
...
from tqec.compile.convention import FIXED_BULK_CONVENTION, DIAGONAL_SCHEDULE_CONVENTION
import sinter
try:
    import pymatching
...
def create_standard_memory_circuit(k=2):
    """Create the standard memory experiment circuit using fixed_bulk convention."""
    print("Creating standard memory experiment circuit...")
    
    # Create memory block graph
    mem_graph = memory(Basis.Z)
    
    # Compile with fixed_bulk convention (default)
...
def create_diagonal_memory_circuit(k=2):
    """Create the diagonal memory experiment circuit using diagonal_schedule convention."""
    print("Creating diagonal memory experiment circuit with diagonal schedule convention...")
    
    # Create memory block graph
    mem_graph = memory(Basis.Z)
    
    # Compile with diagonal_schedule convention
...
def plot_distance_vs_k(distance_data, save_path="distance_vs_k.png"):
    ...
    all_distances = [d for d in standard_distances + diagonal_distances if d is not None]
    if all_distances:
        plt.ylim(min(all_distances) - 0.5, max(all_distances) + 0.5)
    
    # Add annotations for each point
    for i, (k, std_dist, diag_dist) in enumerate(zip(k_values, standard_distances, diagonal_distances)):
        if std_dist is not None:
            plt.annotate(f'{std_dist}', (k, std_dist), textcoords="offset points", 
...
def main():
    """Main comparison function for standard vs diagonal circuits."""
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='Compare standard vs diagonal schedule surface code circuits')
    parser.add_argument('--k-values', nargs='+', type=int, default=[1, 2],
                       help='k values to test (default: 1 2 3)')
    parser.add_argument('--shots', type=int, default=300000000,
                       help='Number of shots for logical error rate calculation (default: 300000000)')
    ...
    print(f"  - Standard: {FIXED_BULK_CONVENTION.name}")
    print(f"  - Diagonal: {DIAGONAL_SCHEDULE_CONVENTION.name}")
    print()
    ...
    if args.noise_levels:
        ...
        noise_levels = np.logspace(-3.5, -2, 4)  # Default: 4 noise levels from 0.0001 to 0.01
    ...
    for k in k_values:
        ...
        standard_circuit = create_standard_memory_circuit(k)
