#!/bin/bash
# Submit benchmark jobs for spatial hadamard experiment with interface_only noise

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
cd "$PROJECT_DIR"

mkdir -p cluster_benchmark/log cluster_benchmark/error cluster_benchmark/output/spatial_hadamard_interface
...
echo "Submitting $TOTAL_JOBS jobs for spatial hadamard (interface_only) experiment..."
...
JOB_ID_FILE="cluster_benchmark/job_ids_spatial_hadamard_interface.txt"
touch "$JOB_ID_FILE"
...
for k in "${K_VALUES[@]}"; do
    for flag_config in "${FLAG_CONFIGS[@]}"; do
        for noise in "${NOISE_VALUES[@]}"; do
            ...
            OUTPUT_FILE="cluster_benchmark/output/spatial_hadamard_interface/result_spatial_hadamard_interface_${flag_config}_k${k}_p${noise}.csv"
            JOB_NAME="sh_int_${flag_config:0:4}_k${k}"
...
WRAPPER_EOF
            ...
            # Track job ID
            if [ -n "$JOB_ID" ]; then
                echo "$JOB_ID spatial_hadamard_interface $flag_config $k $noise" >> "$JOB_ID_FILE"
            fi
