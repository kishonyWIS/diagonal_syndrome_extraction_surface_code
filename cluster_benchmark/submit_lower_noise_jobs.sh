#!/bin/bash
# Submit benchmark jobs for two additional lower noise values
...
DECODERS=("tesseract")
K_VALUES=(1 2 3 4)
# Two new lower noise values: 10^(-3.75) and 10^(-4)
NOISE_VALUES=(0.00017782794100389228 0.0001)
FLAG_CONFIGS=("none" "partial" "all")
DIRECTIONS=("y")

TOTAL_JOBS=$((${#DECODERS[@]} * ${#K_VALUES[@]} * ${#NOISE_VALUES[@]} * ${#FLAG_CONFIGS[@]} * ${#DIRECTIONS[@]}))

echo "Submitting $TOTAL_JOBS jobs for lower noise values..."
