#!/bin/bash
# Submit benchmark jobs for pymatching and correlated_pymatching decoders
...
echo "Submitting $TOTAL_JOBS jobs for pymatching decoders..."
        ...
        for noise in "${NOISE_VALUES[@]}"; do
            for flag_config in "${FLAG_CONFIGS[@]}"; do
                for direction in "${DIRECTIONS[@]}"; do
                    ...
                    echo "[$JOB_NUM/$TOTAL_JOBS] Submitting: decoder=$decoder k=$k noise=$noise flag=$flag_config"
