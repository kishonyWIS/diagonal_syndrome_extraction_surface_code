#!/bin/bash
# Submit benchmark jobs for patch rotation experiment

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
cd "$PROJECT_DIR"

mkdir -p cluster_benchmark/log cluster_benchmark/error cluster_benchmark/output/patch_rotation
...
echo "Submitting $TOTAL_JOBS jobs for patch rotation experiment..."
...
JOB_ID_FILE="cluster_benchmark/job_ids_patch_rotation.txt"
touch "$JOB_ID_FILE"
...
for k in "${K_VALUES[@]}"; do
    for noise in "${NOISE_VALUES[@]}"; do
        ...
        OUTPUT_FILE="cluster_benchmark/output/patch_rotation/result_patch_rotation_k${k}_p${noise}.csv"
        JOB_NAME="pr_k${k}"
        
        echo "[$JOB_NUM/$TOTAL_JOBS] Submitting: k=$k noise=$noise"
...
cd "$PROJECT_DIR_ABS"
export EXPERIMENT_TYPE=patch_rotation
export SCHEDULE=
export K_VAL=$k
export NOISE=$noise
export NOISE_MODE=
...
WRAPPER_EOF
        ...
        # Track job ID
        if [ -n "$JOB_ID" ]; then
            echo "$JOB_ID patch_rotation $k $noise" >> "$JOB_ID_FILE"
        fi
