#!/bin/bash
# Submit benchmark jobs for memory experiment (both N/Z and diagonal schedules)

SCRIPT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
cd "$PROJECT_DIR"

mkdir -p cluster_benchmark/log cluster_benchmark/error cluster_benchmark/output/memory
...
echo "Submitting $TOTAL_JOBS jobs for memory experiment..."
...
JOB_ID_FILE="cluster_benchmark/job_ids_memory.txt"
touch "$JOB_ID_FILE"
...
for schedule in "${SCHEDULES[@]}"; do
    for k in "${K_VALUES[@]}"; do
        for noise in "${NOISE_VALUES[@]}"; do
            ...
            OUTPUT_FILE="cluster_benchmark/output/memory/result_memory_${schedule_suffix}_k${k}_p${noise}.csv"
            JOB_NAME="mem_${schedule_suffix:0:3}_k${k}"
...
WRAPPER_EOF
            ...
            # Track job ID
            if [ -n "$JOB_ID" ]; then
                echo "$JOB_ID memory $schedule $k $noise" >> "$JOB_ID_FILE"
            fi
